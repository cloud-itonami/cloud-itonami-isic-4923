(ns roadfreightops.phase
  "Phase 0->3 staged rollout for the ISIC-4923 'Freight transport by road'
  (trucking/cargo) DISPATCH/LOGISTICS-SCHEDULING operations-coordination
  actor.
  ...
  rollout milestone still to come. Flagging a load-safety/hazmat/
  hours-of-service concern always needs a human to actually look at it --
  a 'flag a concern' op must always escalate and never be auto-commit-
  eligible. `roadfreightops.governor`'s own `always-escalate-ops`
  enforces the same invariant independently -- two layers, not one,
  agree on this."
  (:require [roadfreightops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-shipment-record :schedule-dispatch-operation
                                                 :coordinate-maintenance-order}                                 :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-shipment-record :schedule-dispatch-operation :coordinate-maintenance-order}}})
...
(defn verdict->disposition
  "Map a RoadFreightDispatchGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
