(ns roadfreightops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (RoadFreightDispatchAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the RoadFreightDispatchGovernor
  ...
  human dispatcher/logistics coordinator. The approver resumes with
  ...
  (:require [langgraph.graph :as g]
            ...
            [roadfreightops.governor :as governor]
            [roadfreightops.phase :as phase]
...
(defn build
  ...
  `roadfreightops.store/Store`).
  opts:
    :advisor      -- a `roadfreightops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RoadFreightDispatchAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; RoadFreightDispatchGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :carrier-id (:carrier-id request)
                        :reason (or reason
      ...
      ;; Approval handoff -- paused by interrupt-before; a human dispatcher
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :carrier-id (:carrier-id request) :by (:by approval)}]}
            {:disposition :hold
